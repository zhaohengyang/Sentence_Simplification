...
from headline_generator_tensorflow.python import rnn
...
def _extract_argmax_and_embed(embedding, output_projection=None,
    ...
    def loop_function(prev, _):
        if output_projection is not None:
            # prev.shape [batch_size, hidden_size], output_projection.shape [hidden_size, vocab_size]
...
def _copynet_extract_argmax_and_embed(embedding, output_projection=None,
                              update_embedding=True):
    pass
...
def embedding_attention_decoder(decoder_inputs, initial_state, attention_states,
    ...
    Args:
      ...
      initial_state_attention: If False (default), initial attentions are zero.
        ...
        states -- useful when we wish to resume decoding from a previously
        stored decoder state and attention states.
      copynet: If the decoder contains copynet
    Returns:
      A tuple of the form (outputs, state), where:
        outputs: A list of the same length as decoder_inputs of 2D Tensors with
          shape [batch_size x output_size] containing the generated outputs.
    ...
    # todo added initializer
    with variable_scope.variable_scope(scope or "embedding_attention_decoder"):
        # with ops.device("/cpu:0"):
        if initializer:
            embedding_initializer = initializer
        elif variable_scope.get_variable_scope().initializer:
            embedding_initializer = variable_scope.get_variable_scope().initializer
        else:
            # Default initializer for embeddings should have variance=1.
            sqrt3 = math.sqrt(3)  # Uniform(-sqrt(3), sqrt(3)) has variance=1.
            embedding_initializer = init_ops.random_uniform_initializer(-sqrt3, sqrt3)
        embedding = variable_scope.get_variable("embedding", [num_symbols, embedding_size],
                                                initializer=embedding_initializer)
        if copynet:
            loop_function = _copynet_extract_argmax_and_embed(
                embedding, output_projection,
                update_embedding_for_previous) if feed_previous else None
            emb_inp = [
            embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs]
        ...
        # [tf.histogram_summary(inp.name, inp) for inp in emb_inp]
...
#         encoder_cell = rnn_cell.EmbeddingWrapper(
#             cell, embedding_classes=num_encoder_symbols,
#             embedding_size=embedding_size, initializer=embedding_initializer)
...
# modified
# todo: embedding_attention_seq2seq: added embedding_initializer
# todo: embedding_attention_seq2seq: added encoder_outputs in return
# todo: changed output mapping to word instead of output logits for
def embedding_attention_seq2seq(encoder_inputs, decoder_inputs, cell,
                                num_encoder_symbols, num_decoder_symbols,
                                embedding_size,
                                num_heads=1, output_projection=None,
    ...
    Args:
      ...
      initial_state_attention: If False (default), initial attentions are zero.
        If True, initialize the attentions from the initial state and attention
        states.
      embedding_initializer: use initilializor instead of randome value in embedding

    Returns:
      A tuple of the form (encoder_outputs, outputs, state), where:
        encoder_outputs: A list of outputs (one for each encoder input)
        outputs: A list of the same length as decoder_inputs of 2D Tensors with
          shape [batch_size x num_decoder_symbols] containing the generated
          outputs.
        state: The state of each decoder cell at the final time-step.
    ...
    with variable_scope.variable_scope(scope or "embedding_attention_seq2seq"):
        # Encoder.
        # todo: called with embedding_initializer
        encoder_cell = rnn_cell.EmbeddingWrapper(
            cell=cell, embedding_classes=num_encoder_symbols,
            embedding_size=embedding_size, initializer=embedding_initializer)
        encoder_outputs, encoder_state = rnn.rnn(
            encoder_cell, encoder_inputs, dtype=dtype)
        ...
        outputs_and_state = control_flow_ops.cond(feed_previous,
                                                  lambda: decoder(True),
                                                  lambda: decoder(False))
        return encoder_outputs, outputs_and_state[:-1], outputs_and_state[-1]
    ...
    with ops.op_scope(all_inputs, name, "model_with_buckets"):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                ...
                memory, bucket_outputs, state = seq2seq(encoder_inputs[:bucket[0]],
                ...
                if per_example_loss:
                    ...
                    losses.append(sequence_loss(
                        outputs[-1], targets[:bucket[1]], weights[:bucket[1]],
                        softmax_loss_function=softmax_loss_function))
    print("lengh of outputs:%d" %len(outputs))
    for item in outputs:
        print(len(item))
    return outputs, losses


def model_with_buckets_copynet(encoder_inputs, decoder_inputs, targets, weights,
                       buckets, seq2seq, softmax_loss_function_wrapper=None,
                       per_example_loss=False, copynet=False, name=None):
    ...
    Args:
      ...
      seq2seq: A sequence-to-sequence model function; it takes 2 input that
        agree with encoder_inputs and decoder_inputs, and returns a pair
        consisting of outputs and states (as, e.g., basic_rnn_seq2seq).
      softmax_loss_function_wrapper: Function (inputs-batch, labels-batch) -> loss-batch
        to be used instead of the standard softmax (the default if this is None).
      per_example_loss: Boolean. If set, the returned loss will be a batch-sized
        tensor of losses for each sequence in the batch. If unset, it will be
        a scalar with the averaged loss from all examples.
        ...
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                ...
                memory, bucket_outputs, state = seq2seq(encoder_inputs[:bucket[0]],
                                                decoder_inputs[:bucket[1]])
                if copynet:
                    # todo add softmax_loss_function start
                    softmax_loss_function = None
                    if softmax_loss_function_wrapper:
                        def softmax_loss(inputs, labels):
                            return softmax_loss_function_wrapper(inputs = inputs,
                                                                 labels = labels,
                                                                 memory = memory,
                                                                 encoder_size = bucket[0],
                                                                 encoder_inputs = encoder_inputs[:bucket[0]])
                        softmax_loss_function = softmax_loss
                    # todo add softmax_loss_function end
                else:
                    print("debug, model_with_buckets_copynet():")
    ...
    for item in outputs:
        print(len(item))
    # return outputs logits, and losses
    return outputs, losses
