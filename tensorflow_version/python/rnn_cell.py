...
from headline_generator_tensorflow.python.tensorboard_util import variable_summaries
...
class GRUCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None, summary_op=False):
        ...
        with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
            ...
            with vs.variable_scope("Candidate"):
                c = self._activation(_linear([inputs, r * state],
                                             self._num_units, True, scope="hidden_state_weight", summary_op=summary_op))
            new_h = u * state + (1 - u) * c
        return new_h, new_h
...
class BasicLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None, summary_op=False):
        ...
        with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
            ...
            concat = _linear([inputs, h], 4 * self._num_units, True, summary_op=summary_op)
...
class LSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None, summary_op=False):
        ...
        Args:
          ...
          summary_op: boolean control over if adding tensorboard summary or not
        Returns:
          A tuple containing:
    ...
    def __call__(self, inputs, state, scope=None, summary_op=False):
        ...
        with vs.variable_scope(scope or type(self).__name__):  # "EmbeddingWrapper"
            with ops.device("/cpu:0"):
                ...
                if summary_op:
                    variable_summaries(embedding, "encoder_embedding_matrix")
        return self._cell(embedded, state, summary_op=summary_op)
        ...
        with vs.variable_scope(scope or type(self).__name__):  # "MultiRNNCell"
            ...
            for i, cell in enumerate(self._cells):
                with vs.variable_scope("Cell%d" % i):
                    ...
                    # todo add different scope each layer
                    cur_inp, new_state = cell(cur_inp, cur_state, scope=type(cell).__name__ + "_layer_%d" % i,
...
def _linear(args, output_size, bias, bias_start=0.0, scope=None, summary_op=False):
    ...
    with vs.variable_scope(scope or "Linear"):
        ...
        # todo record matrix summary
        if summary_op:
            variable_summaries(matrix, matrix.name)
        if len(args) == 1:
        ...
        # todo record matrix summary
        if summary_op:
            variable_summaries(bias_term, bias_term.name)
